...
# example:
# ./addCDD2genbank.pl -i RSB205.gbk -o RSB205_pfam.gbk -c /my/path/to/cdd_delta -d pfam
...
our $VERSION = '1.1';   # Feb 10th, 2018; excluded $inference, to make insdc-compliant; see http://www.insdc.org/documents/feature_table.html#5; Feature Key misc_feature; see S_malt_D457 genbank
                        #   increased default qcov_hsp_perc = 100; fixed db_xref (added to annonymous array); 
			#   changed:  note = "Region similar to domain $domain; length:$length, gap_open:$gap_open, evalue:$evalue";
                        # Feb. 9th, 2018; added strand to Bio::SeqFeature::Generic->new; $seq->add_SeqFeature($misc_feat)
...
my ($INP_CDD,$INP_threads,$INP_evalue,$INP_blastpath,$INP_infile,$INP_outfile,$INP_domaindb) = ('cdd_delta',2);
my ($start,$end,$strand,$pepseq,$domstart,$domend);
my ($inference,$note,$cddid,$domain,$function,$length,$gap_open,$evalue,$annot,$db,$tag,$value,$db_xref);
my (%opts,%annotations,@domaindb_names,@trash);

# set defaults
my $max_target_seqs = 15; # may reduce to 7 or so
my $max_hsps        = 1;
my $qcov_hsp_perc   = 100; # may relax, if required
...
if(($opts{'h'})||(scalar(keys(%opts))==0))
{
  ...
  print "-e max E-value of CDD domain matches      [def: $INP_evalue]\n";
  print "-n max_target_seqs                        [def: $max_target_seqs]\n";
  print "-m max num_HSPs                           [def: $max_hsps]       \n";
  print "-q min qcov_hsp_perc                      [def: $qcov_hsp_perc]  \n";
  print "-d take only domains from selected databases  (optional, example -d pfam,smart,TIGR,cd,COG,PRK)\n";
  print "-t number of threads/CPU cores            [def: $INP_threads]\n";
  exit(-1);
}
...
my $cmd;
$cmd = "$DELTABLAST -num_threads $INP_threads -query $tmpfilename -db $tmpfilename ".
  "-max_target_seqs $max_target_seqs -max_hsps $max_hsps -qcov_hsp_perc $qcov_hsp_perc ".
  "-rpsdb $INP_CDD -evalue $INP_evalue -show_domain_hits -outfmt \"6 qaccver saccver pident length mismatch gapopen qstart qend sstart send evalue bitscore qlen stitle qcovhsp\"";
...
while(my $line = <DELTA>)
{
  #qaccver saccver pident length mismatch gapopen qstart qend sstart send evalue bitscore qlen stitle qcovhsp
  ...
  $db_xref = (split(/\|/,$data[1],3))[-1];
  $domain = (split(/,/,$data[13],2))[0];
  $length = $data[3];
  $gap_open = $data[5];
  $evalue = $data[10];
  $note = "Region similar to domain $domain; length:$length, gap_open:$gap_open, evalue:$evalue";
  $function = (split(/,/,$data[13],2))[1];
  $function =~ s/^\h+//;
  chomp($function);
  ...
  #$note = $data[13];
  #push( @{$annotations{$strand}{$start}{$end}}, [ $domstart,$domend,$inference,$db_xref,$note ] );
  push( @{$annotations{$strand}{$start}{$end}}, [ $domstart,$domend,$db_xref,$note,$function ] );
  $annotated_domains++;
}
close(DELTA);
...
while( my $seq = $ingbk->next_seq())
{
  foreach my $f ($seq->get_SeqFeatures)
  {
    ...
    if($annotations{$strand}{$start}{$end})
    {
      ...
      {
        #($domstart,$domend,$inference,$db_xref,$note) = (@$annot);
	($domstart,$domend,$db_xref,$note,$function) = (@$annot);
